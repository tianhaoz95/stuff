...
pub async fn engine_loop(
    ...
    loop {
        ...
        if let Some(batch) = maybe_batch {
            ...
            // No runnable requests; yield to allow new ones to arrive.
...
In practice, the main loop looks like:
...
pub async fn run_overlapped_loop(
    ...
    gpu_rx: tokio::sync::mpsc::Receiver<GpuResult>,
) {
    let mut pending_result: Option<(Batch, tokio::sync::oneshot::Receiver<GpuResult>)> = None;
    ...
    loop {
        ...
        // If we have a pending GPU result, wait for it.
        if let Some((prev_batch, result_rx)) = pending_result.take() {
            let gpu_result = result_rx.await.expect("GPU thread dropped sender");
            process_logits(&mut ctx, &prev_batch, gpu_result.logits).await;
            // Transition prefilling requests that completed to decoding.
            finalize_prefill_requests(&mut ctx, &prev_batch);
        }

        // Schedule the next batch (CPU work, concurrent with GPU if we
        // send the previous batch asynchronously).
        let maybe_batch = schedule_batch(&mut ctx);

        if let Some(batch) = maybe_batch {
            // Fire prefetch I/O for the next batch's weights/KV.
            let _ = ctx.backend.prefetch_next(&batch).await;

            // Send to GPU thread; get back a oneshot receiver for the result.
            let (result_tx, result_rx) = tokio::sync::oneshot::channel();
            gpu_tx.send(GpuWork { batch: batch.clone(), result_tx })
                .expect("GPU thread dropped receiver");

            pending_result = Some((batch, result_rx));
        } else {
            tokio::task::yield_now().await;
        }
    }
...
The radix cache is a prefix tree over token sequences. Each leaf or internal node stores a set of KV cache pages for the token prefix that leads to it. When a new request arrives, we walk the tree to find the longest matching prefix; the matching portion does not need to be recomputed.
...
pub struct RadixNode {
    /// The token sequence stored *at this edge* (not the full path from root).
    /// An edge from parent to this node corresponds to `edge_tokens`.
    pub edge_tokens: Vec<u32>,

    /// KV cache pages storing the KV for `edge_tokens`, appended to the
    /// parent's KV sequence. Only full pages are stored here (partial pages
    /// are not cached — they belong to the request and will be promoted
    /// when the page fills).
    pub kv_pages: Vec<PageIndex>,

    /// Child nodes, keyed by the first token of their edge.
    pub children: HashMap<u32, Arc<parking_lot::RwLock<RadixNode>>>,

    /// Reference count: number of in-flight requests that are using this node.
    /// A node with ref_count > 0 cannot be evicted.
    pub ref_count: usize,

    /// LRU timestamp: updated to `now()` whenever a request is matched here
    /// or a child is inserted. Nodes with lower timestamps are evicted first.
    pub last_access: std::time::Instant,

    /// Total number of KV pages held by this subtree (self + all descendants).
    /// Used for fast eviction budget accounting.
    pub subtree_pages: usize,
}
...
pub struct RadixCache {
    pub root: Arc<parking_lot::RwLock<RadixNode>>,
    /// How many tokens each KV page stores.
    pub page_size: usize,
    /// Total number of pages currently held across all nodes.
    pub total_pages: usize,
    /// Maximum pages before eviction is triggered.
    pub max_pages: usize,
}
...
impl RadixCache {
    pub fn new(page_size: usize, max_pages: usize) -> Self {
        RadixCache {
            root: Arc::new(parking_lot::RwLock::new(RadixNode::new_root())),
            page_size,
            total_pages: 0,
            max_pages,
        }
    ...
    /// Walk the tree and return the number of tokens matched by the longest
    /// cached prefix of `tokens`. Also increments ref_count on matched nodes
    /// to prevent eviction while the request is live.
    /// Returns (matched_len, list of page indices).
    pub fn match_prefix(&self, tokens: &[u32]) -> (usize, Vec<PageIndex>) {
        let mut matched_pages = Vec::new();
        let mut matched_tokens = 0;
        let mut node_lock = self.root.read();

        loop {
            let first_token = match tokens.get(matched_tokens) {
                Some(&t) => t,
                None => break,
            };

            let child_arc = match node_lock.children.get(&first_token) {
                Some(c) => c.clone(),
                None => break,
            };
            ...
            // Check how many edge tokens match.
            ...
            // Only count full pages (partial pages at the end are not cached).
            let full_pages = match_len / self.page_size;
            if full_pages == 0 {
                break;
            }

            matched_pages.extend_from_slice(&child.kv_pages[..full_pages]);
            matched_tokens += full_pages * self.page_size;

            // If we matched the full edge, descend further.
            if match_len == child.edge_tokens.len() {
                drop(child);
                let child_write = child_arc.write();
                // (bump last_access — in production use interior mutability)
                drop(child_write);
                node_lock = child_arc.read();
            } else {
    ...
    /// After a request finishes prefill, insert the newly computed KV pages
    /// into the tree so future requests can reuse them.
    /// `tokens` is the full token sequence; `pages` are the new pages computed.
    /// Only complete pages (page_size tokens) are inserted.
    pub fn insert_prefix(
        ...
        let mut node_arc = self.root.clone();
        ...
        loop {
            ...
            let has_child = {
                ...
                node.children.contains_key(&first_token)
            };

            if has_child {
                let child_arc = node_arc.read().children[&first_token].clone();
                let edge_len = child_arc.read().edge_tokens.len();
                // Find split point.
                let remaining = &tokens[offset..];
                let match_len = child_arc
                    .read()
                    .edge_tokens
                    .iter()
                    .zip(remaining)
                ...
                if match_len == edge_len {
                    ...
                    self.split_edge(node_arc.clone(), &child_arc, match_len);
                    // After split, the matched prefix is a new internal node.
                    let new_child = node_arc.read().children[&first_token].clone();
                    offset += match_len;
                    node_arc = new_child;
                }
            } else {
                // No matching child: insert a new leaf.
                let new_tokens = tokens[offset..].to_vec();
                let new_pages = pages[(offset / self.page_size)..].to_vec();
                let new_node = Arc::new(parking_lot::RwLock::new(RadixNode {
                    edge_tokens: new_tokens,
                    kv_pages: new_pages,
                    children: HashMap::new(),
                    ref_count: 0,
                    last_access: std::time::Instant::now(),
                    subtree_pages: pages[(offset / self.page_size)..].len(),
                }));
                node_arc.write().children.insert(first_token, new_node);
                self.total_pages += pages[(offset / self.page_size)..].len();
                break;
            }
        }
    }

    /// Evict least-recently-used nodes to free at least `target_pages` pages.
    /// Returns the number of pages actually freed.
    pub fn evict_pages(&mut self, target_pages: usize) -> usize {
        let mut freed = 0usize;
        // Collect all evictable leaves (ref_count == 0) into a min-heap
        // ordered by last_access.
        let mut candidates: Vec<(std::time::Instant, Arc<parking_lot::RwLock<RadixNode>>)> = Vec::new();
        self.collect_evictable_leaves(&self.root.clone(), &mut candidates);
        candidates.sort_by_key(|(t, _)| *t);

        for (_, node_arc) in candidates {
            if freed >= target_pages {
                break;
            }
            let pages_held = node_arc.read().kv_pages.len();
            // Remove from parent (not shown for brevity; tracked via parent pointer or re-walk).
            freed += pages_held;
            self.total_pages -= pages_held;
        }
        freed
    }
    ...
    fn collect_evictable_leaves(
        &self,
        node_arc: &Arc<parking_lot::RwLock<RadixNode>>,
        out: &mut Vec<(std::time::Instant, Arc<parking_lot::RwLock<RadixNode>>)>,
    ) {
        let node = node_arc.read();
        if node.children.is_empty() && node.ref_count == 0 && !node.kv_pages.is_empty() {
            out.push((node.last_access, node_arc.clone()));
        }
        for child in node.children.values() {
            self.collect_evictable_leaves(child, out);
        }
    }

    fn split_edge(
        &self,
        parent_arc: Arc<parking_lot::RwLock<RadixNode>>,
        child_arc: &Arc<parking_lot::RwLock<RadixNode>>,
        split_at: usize,
    ) {
        // Split child's edge at split_at tokens.
        // New internal node takes edge_tokens[..split_at] and kv_pages[..split_at/page_size].
        // Child keeps edge_tokens[split_at..] and kv_pages[split_at/page_size..].
        let (prefix_tokens, suffix_tokens, prefix_pages, suffix_pages) = {
            let child = child_arc.read();
            let page_split = split_at / self.page_size;
            (
                child.edge_tokens[..split_at].to_vec(),
                child.edge_tokens[split_at..].to_vec(),
                child.kv_pages[..page_split].to_vec(),
                child.kv_pages[page_split..].to_vec(),
            )
        };

        let first_token_of_suffix = suffix_tokens[0];
        let first_token_of_prefix = prefix_tokens[0];

        // Modify child to hold only the suffix.
        {
            let mut child = child_arc.write();
            child.edge_tokens = suffix_tokens;
            child.kv_pages = suffix_pages;
        }

        // Create new internal node with prefix, pointing to child.
        let mut new_node = RadixNode {
            ...
            ref_count: 0,
            last_access: std::time::Instant::now(),
            subtree_pages: child_arc.read().subtree_pages,
        };
        new_node.children.insert(first_token_of_suffix, child_arc.clone());

        // Replace child in parent.
        parent_arc
            .write()
            .children
            .insert(first_token_of_prefix, Arc::new(parking_lot::RwLock::new(new_node)));
    }
}
```

### 5.3 Lock/Unlock Lifecycle

When a request is matched against the radix cache, the matched nodes are **locked** (ref_count incremented) to prevent eviction while the request is in flight. When the request completes, the nodes are **unlocked**.

```rust
pub fn lock_nodes(nodes: &[Arc<parking_lot::RwLock<RadixNode>>]) {
    for node in nodes {
        node.write().ref_count += 1;
    }
}

pub fn unlock_nodes(nodes: &[Arc<parking_lot::RwLock<RadixNode>>]) {
    for node in nodes {
        let mut n = node.write();
        n.ref_count = n.ref_count.saturating_sub(1);
        n.last_access = std::time::Instant::now();
    }
}
```

### 5.4 Page-Aligned Storage

Only **complete pages** are inserted into the radix cache. A page holds exactly `page_size` tokens. If a request's prompt length is not a multiple of `page_size`, the partial last page is kept in the request's private allocation and is not cached. This simplifies the cache: every cache entry has an integral number of full pages.

```rust
/// Promote a completed request's KV pages into the radix cache.
/// Called after the request transitions from Prefilling to Decoding.
pub fn promote_to_cache(
    cache: &mut RadixCache,
    req: &Request,
) {
    let full_pages = req.device_len / cache.page_size;
    let tokens_to_cache = full_pages * cache.page_size;
    if full_pages == 0 {
        return;
    }

    // Evict if we would exceed capacity.
    let free = cache.max_pages - cache.total_pages;
    let need = full_pages;
    if need > free {
        cache.evict_pages(need - free);
    }

    cache.insert_prefix(
        &req.input_ids[..tokens_to_cache],
        &req.kv_pages[..full_pages],
    );
}
```

### 5.5 Interaction with KvOffloadManager (SSD offload)

When a radix cache node's pages are evicted *and* SSD offloading is enabled, the pages may be written to SSD rather than simply freed. This is transparent to the scheduler: `evict_pages` calls `KvOffloadManager::offload_pages(pages)`, which initiates an async write. The pages remain logically "in the cache" (as SSD-resident) until the SSD write completes. Subsequent cache hits that land on SSD-resident pages trigger a prefetch before the forward pass begins.

```rust
pub fn evict_to_ssd(
    cache: &mut RadixCache,
    kv_offload: &mut KvOffloadManager,
    target_pages: usize,
) -> usize {
    // Collect LRU leaves as before.
    let mut freed = 0;
    // For each evicted page, instead of freeing, offload to SSD.
    // kv_offload.offload_pages returns a future; we fire-and-forget
    // and mark the page as OFFLOADED (not FREE) in the pool.
    // ...
    freed
}
```
...
### Phase 1A — Weeks 1–4: Core Data Structures and Scheduler Skeleton

**Goal:** A working continuous batching loop that can run a real forward pass using a stub backend. No radix cache, no offload, no overlap scheduling.

**Tasks:**

- [ ] Define crate structure: `burn-inference` workspace with sub-crates:
  - `engine/` — scheduler, request lifecycle, KV pool
  - `api/` — HTTP server, tokenizer service
  - `backend/` — `BackendHandle` trait and a `StubBackend` for testing
- [ ] Implement `SamplingParams`, `Request`, `RequestState`, `Batch`, `BatchPhase`
- [ ] Implement `KvCachePool`: fixed-size page allocator (free list, `allocate`, `free`)
- [ ] Implement basic `PageTable`: `HashMap<(RequestId, usize), PageIndex>`
- [ ] Implement `EngineContext`: holds waiting/prefilling/decoding queues
- [ ] Implement `schedule_batch`: naive version without radix cache (assume cache miss on every request)
- [ ] Implement `PrefillAdder` greedy algorithm with token budget
- [ ] Implement `collect_decode_requests`
- [ ] Implement `assemble_batch`
- [ ] Implement `process_logits`: greedy sampling (argmax) only
- [ ] Implement `StubBackend`: returns random logits with a configured latency
- [ ] Write integration test: send 10 concurrent requests with 128-token prompts and 64-token max_new_tokens; verify all complete without deadlock
- [ ] Milestone: `cargo test` passes; stub throughput test shows >0 tokens/s

### Phase 1B — Weeks 5–8: Radix Cache, Chunked Prefill, Overlap Scheduling

**Goal:** Radix cache reduces redundant KV computation; chunked prefill handles 4096+ token prompts; overlap scheduling hides scheduling latency.

**Tasks:**

- [ ] Implement `RadixNode` and `RadixCache` (§5): `match_prefix`, `insert_prefix`, `evict_pages`
- [ ] Implement `split_edge` for partial prefix matches
- [ ] Implement `lock_nodes` / `unlock_nodes` lifecycle
- [ ] Implement `promote_to_cache`: called when prefill completes
- [ ] Wire radix cache into `schedule_batch`: call `match_prefix` on admission; set `cached_len`
- [ ] Implement chunked prefill: requests with `input_ids.len() > max_prefill_tokens_per_iter` are processed over multiple iterations
- [ ] Update `PrefillAdder` to handle cross-iteration prefill state
- [ ] Implement overlap scheduling (§4.6): `run_overlapped_loop`
- [ ] Implement `preempt_one_prefill_request` (§4.7)
- [ ] Implement `temperature` + `top_p` + `top_k` sampling (nucleus sampling)
- [ ] Implement `repetition_penalty`
- [ ] Implement stop sequence detection
- [ ] Write unit tests:
  - [ ] `test_radix_cache_match_miss`: new request, no cache hit
  - [ ] `test_radix_cache_match_hit`: two requests sharing 512-token system prompt; second request should have 512 tokens cached
  - [ ] `test_radix_cache_eviction`: fill cache beyond capacity; verify LRU eviction
  - [ ] `test_chunked_prefill`: 4096-token prompt with 1024-token budget; verify 4 iterations
  - [ ] `test_overlap_scheduling`: verify CPU scheduling and I/O overlap with GPU (using mock timing)
- [ ] Milestone: radix cache hit rate > 80% on repeated-system-prompt benchmark; chunked prefill passes correctness tests

### Phase 1C — Weeks 9–12: Integration with burn-wgpu Backend (Linux)

**Goal:** Run real inference on a Linux system with Intel Arc iGPU. Full stack: HTTP → tokenizer → scheduler → burn-wgpu backend.

**Tasks:**

- [ ] Implement `WgpuBackendHandle`: wraps `burn-wgpu` forward pass as `BackendHandle`
- [ ] Implement attention kernel integration: pass `PagedAttnMetadata` to burn-wgpu attention op
- [ ] Integrate `WeightCache` from burn-wgpu: call `prefetch_weights` in overlap loop
- [ ] Integrate `ExpertWeightCache` for MoE model (26B MoE): `prefetch_expert_layer`
- [ ] Integrate `KvOffloadManager`: handle `PageState::Offloaded` in `collect_offloaded_pages`
- [ ] Implement `UnifiedPageAllocator` (§6.4): local + global pools, SSD fallback
- [ ] Implement `PrefetchOps` integration (§9.3): `prefetch_weights`, `prefetch_kv_pages`, `wait_prefetch`, `launch_forward`
- [ ] Implement FlashMoE integration (§8): routing before FFN, expert prefetch, MoE-aware batching
- [ ] Start tokenizer service; wire HTTP API (§10) end-to-end
- [ ] Correctness test on real hardware:
  - [ ] `"What is the capital of China?"` → `output.to_lowercase().contains("beijing")`
  - [ ] `"What is 1+1? Answer with only a number."` → `output.trim() == "2"`
- [ ] Milestone: full end-to-end inference working on Linux; TTFT < 5s for 512-token prompt on 26B MoE; 5+ tokens/s decode throughput at batch size 1

### Phase 2A — Weeks 13–16: burn-ggml Backend Integration (macOS)

**Goal:** Run inference on macOS with Metal. Same scheduler, different backend.

**Tasks:**

- [ ] Implement `GgmlBackendHandle`: wraps `burn-ggml` forward pass as `BackendHandle`
- [ ] Implement Metal-specific paged attention (if ggml's attention kernel supports it natively; otherwise implement a gather-then-attend approach)
- [ ] Integrate `WeightCache` (layer-wise weight streaming for 31B dense model)
- [ ] Integrate `KvOffloadManager` for long-context (global attention) layers
- [ ] Tune KV pool size for 32 GB M3/M4: target 4–8 GB KV pool
- [ ] Wire up `PrefetchOps` with macOS kqueue-based async I/O
- [ ] Correctness tests on macOS:
  - [ ] Same two tests as Phase 1C
  - [ ] Long context test: 8192-token prompt, verify full KV chain is correct
- [ ] Performance: measure tokens/s on 31B dense Q4 at batch size 1, 4, 8
- [ ] Milestone: full end-to-end inference working on macOS; >10 tokens/s at batch size 1 for 31B Q4

### Phase 2B — Weeks 17–20: Disaggregation, API Polish, Benchmarking

**Goal:** Prefill-decode disaggregation architecture; production-grade API; benchmark suite.

**Tasks:**

- [ ] Implement `PrefillWorker` and `DecodeWorker` tasks (§7.2)
- [ ] Implement `KvHandoffQueue` (§7.3): lock-free SPSC ring buffer for KV page handoff
- [ ] Implement `SharedGpuBackend`: mutex-based GPU time sharing between workers
- [ ] Wire disaggregation into the engine loop
- [ ] API polish:
  - [ ] CORS support via `tower-http`
  - [ ] Request timeout handling
  - [ ] Graceful shutdown: drain in-flight requests before stopping
  - [ ] `/v1/completions` endpoint (legacy, non-chat)
  - [ ] Prometheus metrics endpoint `/metrics`
- [ ] Benchmark suite:
  - [ ] Single-request TTFT benchmark (cold cache)
  - [ ] Single-request TTFT benchmark (warm cache, radix hit)
  - [ ] Concurrent requests throughput benchmark (1, 4, 8, 16, 32 concurrent)
  - [ ] Shared-prefix benchmark: 100 requests with 1024-token shared system prompt
  - [ ] Long-context benchmark: 8192-token context
- [ ] Milestone: all performance targets in §14 achieved; benchmark results documented
...
### 14.1 Phase 1C Targets: Linux (26B MoE, Intel Arc iGPU)
...
### 14.2 Phase 2A Targets: macOS (31B Dense, Apple Silicon)
...
### 14.3 Phase 2B Targets: Disaggregated Architecture
...
**Probability**: Medium. Requires careful implementation in Phase 1A but the pattern is well-established in projects like mistral.rs.
